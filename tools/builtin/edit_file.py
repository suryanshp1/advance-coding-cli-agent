...
from safety.approval import ToolConfirmation
...
class EditParams(BaseModel):
    ...
    old_string: str = Field(
        ...,
        description="The exact text to find and replace. Must match exactly including whitespace and indentation. For new files this will be empty.",
    )
...
class EditTool(Tool):
    ...
    async def get_confirmation(
        ...
        if is_new_file:
            ...
            return ToolConfirmation(
                ...
                description=f"Create new file: {params.path}",
                diff=diff,
                affected_paths=[path],
            )
        ...
        return ToolConfirmation(
            tool_name=self.name,
            params=invocation.params,
            description=f"Edit file: {params.path}",
            diff=FileDiff(
                path=path,
    ...
    async def execute(self, invocation: ToolInvocation) -> ToolResult:
        ...
        if not path.exists():
            if params.old_string:
                return ToolResult.error_result(
                    f"Cannot edit non-existent file '{path}'. To create a new file use empty old_string."
                )
            ...
            return ToolResult.success_result(
                f"Created new file '{path}' with {line_count} lines.",
                diff=FileDiff(
                    path=path,
                    old_content="",
                    new_content=params.new_string,
        ...
        if not params.old_string:
            return ToolResult.error_result(
                "old_string is empty but fiel exists. Provide old_string to edit or use write_file to overwrite."
            )
        ...
        if params.replace_all:
            new_content = old_content.replace(params.old_string, params.new_string)
            replaced_count = occurance_count
        else:
            new_content = old_content.replace(params.old_string, params.new_string, 1)
            replaced_count = 1
        ...
        if new_content == old_content:
            return ToolResult.error_result(
                f"No changes made to '{path}'. The old_string equals new_string."
            )

        try:
            path.write_text(new_content, encoding="utf-8")
        except Exception as e:
            return ToolResult.error_result(f"Failed to write to file '{path}': {e}")
        ...
        return ToolResult.success_result(
            f"Edited {path}: Replaced {replaced_count} occurrence(s) {diff_msg}.",
            diff=FileDiff(
                path=path,
                old_content=old_content,
                new_content=new_content,
